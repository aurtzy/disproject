...
For compilation or shell commands, Disproject provides suffixes
are designed to be customized via keyword values in suffix
specifications.  These can be found in:
  `M-x shortdoc disproject-custom-suffixes'
Alternatively, if more flexibility is needed, see
  `M-x shortdoc disproject-environment'
for functions that automatically set dynamic variables according
to the transient state.

An example using some of the various methods described are
provided below.  Note that some of the examples may use the
`advice' or `advice*' slots, which is an experimental feature in
recent Transient versions.
  ...
  ((\"f\" \"Find a file from project\"
   ...
   (\"M\" \"Run make test\" disproject-compile
    ...
    :buffer-id \"make\")
   (\"s\" \"Sleep a bit (allow simultaneous buffers)\" disproject-shell-command
    :cmd \"\\
echo You can start another buffer before this completes...
sleep 5
echo Done sleeping.\"
...
(define-short-documentation-group disproject-custom-suffixes
  "Customizable suffixes for `disproject-custom-suffixes'
(Use `C-h o' on symbols for more documentation)"
  (disproject-compile)
  (disproject-shell-command))
